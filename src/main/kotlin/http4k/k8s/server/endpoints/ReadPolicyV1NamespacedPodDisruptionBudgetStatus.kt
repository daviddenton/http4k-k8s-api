 ...
 * 		401 Unauthorized
 */
fun ReadPolicyV1NamespacedPodDisruptionBudgetStatus(): RoutingHttpHandler {
	val iok8sapipolicyv1PodDisruptionBudgetLens = Body.auto<PodDisruptionBudget>().toLens()
	val nameLens = Path.string().of("name")
	val namespaceLens = Path.string().of("namespace")
	val prettyLens = Query.string().optional("pretty")

	return "/apis/policy/v1/namespaces/{namespace}/poddisruptionbudgets/{name}/status" bind Method.GET to { req: Request ->
