...
fun ReplaceAdmissionregistrationV1alpha1ValidatingAdmissionPolicyBinding():
    ...
    return "/apis/admissionregistration.k8s.io/v1alpha1/validatingadmissionpolicybindings/{name}" bind Method.PUT to { req: Request ->
        val name = nameLens(req)
        val pretty = prettyLens(req)
        val dryRun = dryRunLens(req)
        val fieldManager = fieldManagerLens(req)
        val fieldValidation = fieldValidationLens(req)
        val iok8sapiadmissionregistrationv1alpha1ValidatingAdmissionPolicyBinding =
            iok8sapiadmissionregistrationv1alpha1ValidatingAdmissionPolicyBindingLens(req)
        Response(Status.OK)
