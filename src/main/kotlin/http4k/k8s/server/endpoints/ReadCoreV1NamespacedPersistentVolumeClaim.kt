 ...
 * 		401 Unauthorized
 */
fun ReadCoreV1NamespacedPersistentVolumeClaim(): RoutingHttpHandler {
	val iok8sapicorev1PersistentVolumeClaimLens = Body.auto<PersistentVolumeClaim>().toLens()
	val nameLens = Path.string().of("name")
	val namespaceLens = Path.string().of("namespace")
	val prettyLens = Query.string().optional("pretty")

	return "/api/v1/namespaces/{namespace}/persistentvolumeclaims/{name}" bind Method.GET to { req: Request ->
