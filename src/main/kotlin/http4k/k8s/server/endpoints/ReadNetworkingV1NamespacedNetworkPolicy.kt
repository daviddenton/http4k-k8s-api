 ...
 * 		401 Unauthorized
 */
fun ReadNetworkingV1NamespacedNetworkPolicy(): RoutingHttpHandler {
    val iok8sapinetworkingv1NetworkPolicyLens = Body.auto<NetworkPolicy>().toLens()
    val nameLens = Path.string().of("name")
    val namespaceLens = Path.string().of("namespace")
    val prettyLens = Query.string().optional("pretty")

    return "/apis/networking.k8s.io/v1/namespaces/{namespace}/networkpolicies/{name}" bind Method.GET to { req: Request ->
