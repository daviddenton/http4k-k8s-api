 ...
 * 		401 Unauthorized
 */
fun ReadCoreV1NamespacedReplicationController(): RoutingHttpHandler {
    val iok8sapicorev1ReplicationControllerLens = Body.auto<ReplicationController>().toLens()
    val nameLens = Path.string().of("name")
    val namespaceLens = Path.string().of("namespace")
    val prettyLens = Query.string().optional("pretty")

    return "/api/v1/namespaces/{namespace}/replicationcontrollers/{name}" bind Method.GET to { req: Request ->
