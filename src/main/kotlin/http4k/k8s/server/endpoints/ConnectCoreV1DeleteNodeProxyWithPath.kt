 ...
 * 		401 Unauthorized
 */
fun ConnectCoreV1DeleteNodeProxyWithPath(): RoutingHttpHandler {
    ...
    return "/api/v1/nodes/{name}/proxy/{path}" bind Method.DELETE to { req: Request ->
        val name = nameLens(req)
        val path = pathLens(req)
        val pathQuery = pathQueryLens(req)
        Response(Status.OK)
            .with(connectCoreV1DeleteNodeProxyWithPathResponse200Lens of TODO())
    }
}
