 ...
 * 		401 Unauthorized
 */
fun ConnectCoreV1PatchNodeProxyWithPath(): RoutingHttpHandler {
    ...
    return "/api/v1/nodes/{name}/proxy/{path}" bind Method.PATCH to { req: Request ->
        val name = nameLens(req)
        val path = pathLens(req)
        val pathQuery = pathQueryLens(req)
        Response(Status.OK)
            .with(connectCoreV1PatchNodeProxyWithPathResponse200Lens of TODO())
    }
}
