 ...
 * 		401 Unauthorized
 */
fun ReadResourceV1alpha1NamespacedPodSchedulingStatus(): RoutingHttpHandler {
    val iok8sapiresourcev1alpha1PodSchedulingLens = Body.auto<PodScheduling>().toLens()
    val nameLens = Path.string().of("name")
    val namespaceLens = Path.string().of("namespace")
    val prettyLens = Query.string().optional("pretty")

    return "/apis/resource.k8s.io/v1alpha1/namespaces/{namespace}/podschedulings/{name}/status" bind Method.GET to { req: Request ->
